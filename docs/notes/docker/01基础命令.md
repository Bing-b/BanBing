...
```shell
# 镜像打包
docker save -o <保存的文件路径/文件名> <镜像名称>
# eg: docker save -o /data/imagse/website_doc.tar website_doc:v1

# 镜像加载
docker load -i <镜像路径/文件名>
# eg: docker load -i website_doc.tar

# 启动镜像
# -d 后台运行
# --name 指定容器名
# -p 指定端口映射，格式：宿主机端口:容器端口
# -v 指定数据卷挂载，格式：宿主机路径:容器路径
# -e 指定环境变量，格式：变量名=变量值
docker run -d --name <容器名> -p <宿主机端口>:<容器端口> -v /本地路径:/容器路径  <镜像名称>
# eg: docker run -d --name website_doc -p 80:80 -v /data/website_doc:/website_doc website_doc:v1
```
